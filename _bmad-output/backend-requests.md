...
**Reported:** 2026-03-05 | **Updated:** 2026-03-25
...
**Status:** Partially resolved — `duplicable_rule` replaced by structured `OccurrenceRule { min, max }`. The null convention question still applies to `hidden_rule`, `required_rule`, `disabled_rule`, `default_value_rule`, `constrained_rule`.

### ~~6. `GET /communities/{id}/users` — missing GET endpoint~~ → CLOSED
**Reported:** 2026-03-05 | **Closed:** 2026-03-25
**Resolution:** `GET /communities/{community_id}/users` now exists in spec.

### ~~7. `technical_label` on `IndicatorModelWithAssociation`~~ → CLOSED
**Reported:** 2026-03-05 | **Closed:** 2026-03-25
**Resolution:** `technical_label` now present on `IndicatorModelWithAssociation`, `IndicatorModelRead`, and new `SectionIndicatorModelRead`.
...
**Update 2026-03-25:** `community_id` filter now available on `GET /users/`. Remaining open: confirm `/auth/users` removal is intentional.
